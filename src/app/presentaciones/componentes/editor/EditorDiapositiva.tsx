...
interface EditorDiapositivaProps {
  ...
  className?: string;
  // Propiedades adicionales para la versión simplificada
...
export function EditorDiapositiva({
  ...
  const extraerVariables = (texto: string | undefined): string[] => {
    if (!texto) return []
  ...
  const mapearVariable = (variable: string, columna: string) => {
    setElementosEditados(prev => prev.map(elemento =>
      elemento.id === variable ? { ...elemento, contenido: columna } : elemento
    ))
  }
  ...
  const aplicarMapeo = () => {
    ...
    const elementosActualizados = elementosEditados.map(elemento => {
      ...
      Object.entries(filaSeleccionada.valores).forEach(([variable, valor]) => {
        if (valor !== undefined) {
  ...
  const guardarCambios = async () => {
    try {
      setCargando(true)
      if (alActualizarElementos) {
        await alActualizarElementos(elementosEditados)
      } else if (alGuardar) {
        await alGuardar(elementosEditados)
      }
    } finally {
      setCargando(false)
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {todasLasVariables.map(variable => (
                <div key={variable} className="space-y-1">
                  ...
                  <Select
                    value={elementosEditados.find(e => e.id === variable)?.contenido || ""}
