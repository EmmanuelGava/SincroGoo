...
import { ListaDiapositivas } from "@/app/presentaciones/componentes/ListaDiapositivas"
...
import { Elemento } from "@/servicios/supabase/globales/tipos"
...
export default function PaginaSincronizar() {
  ...
  const cargarDiapositivas = async (slidesId: string) => {
    try {
      ...
      if (resultado.datos.length > 0) {
        ...
        const elementosResultado = await ElementosAPI.obtenerElementosPorDiapositiva(primeraId)
        if (elementosResultado && elementosResultado.length > 0) {
          setElementos(elementosResultado.map(convertirAElementoDiapositiva))
          
          // Extraer variables de los elementos
          const variables = new Set<string>()
          elementosResultado.forEach((elemento: Elemento) => {
  ...
  const cargarHojaCalculo = async (sheetsId: string) => {
    try {
      ...
      const resultado = await servicioSheets.obtenerDatosHoja(sheetsId)
      if (!resultado.exito || !resultado.datos) {
  ...
  const manejarSeleccionDiapositiva = async (id: string) => {
    try {
      ...
      const slidesId = localStorage.getItem("connectedSlides")
      if (!slidesId) {
        throw new Error("No hay una presentación conectada")
      }
      
      // Cargar elementos de la diapositiva seleccionada
      const resultado = await ElementosAPI.obtenerElementosPorDiapositiva(id)
      if (resultado && resultado.length > 0) {
        setElementos(resultado.map(convertirAElementoDiapositiva))
        
        // Extraer variables de los elementos
        const variables = new Set<string>()
        resultado.forEach((elemento: Elemento) => {
          if (elemento.contenido) {
            extraerVariables(elemento.contenido).forEach(variable => {
              variables.add(variable)
            })
    ...
    try {
      ...
      for (const elemento of elementosActualizados) {
        ...
        if (original && original.contenido !== elemento.contenido) {
          await servicioSlides.actualizarElemento(
            slidesId,
  ...
  const aplicarMapeo = () => {
    ...
    manejarPreview(elementosActualizados)
  }

  // Función para convertir Elemento a ElementoDiapositiva
  const convertirAElementoDiapositiva = (elemento: Elemento): ElementoDiapositiva => {
    return {
      id: elemento.elemento_id,
      tipo: elemento.tipo as ElementoDiapositiva['tipo'],
      contenido: elemento.contenido || '',
      idDiapositiva: elemento.diapositiva_id,
      posicion: elemento.posicion ? {
        x: elemento.posicion.x || 0,
        y: elemento.posicion.y || 0,
        ancho: elemento.posicion.ancho || 0,
        alto: elemento.posicion.alto || 0
      } : undefined,
      estilo: elemento.estilo,
      columnaAsociada: elemento.columnaAsociada || undefined,
      tipoAsociacion: elemento.tipo_asociacion || undefined,
      celdaId: elemento.celda_asociada || undefined
    }
  }
